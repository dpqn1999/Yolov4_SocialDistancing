    ...
    if len(detections) > 0:  						# At least 1 detection in the image and check detection presence in a frame  
        ...
        for detection in detections:				# In this if statement, we filter all the detections for persons only
            ...
            if name_tag == 'person':                
                ...
                pt1 = (xmin, ymin)
                pt2 = (xmax, ymax)
                detection2.append([xmin, ymin, xmax, ymax])
                cv2.putText(img,
                            "",
        ...
        boxes_ids = tracker.update(detection2)
    ...
    #=================================================================            	
        ...
        for idx, box in centroid_dict.items():  # dict (1(key):red(value), 2 blue)  idx - key  box - value
            if idx in red_zone_list:   # if id is in red zone list
                ...
                if (idList[tempindex] not in listid):
                    listid.append(idList[tempindex])
                    ROI = img[box[3]:box[5], box[2]:box[4]]
                    name = 'E:\Output\\' + str(idList[tempindex]) + '.jpg'
                    cv2.imwrite(name, ROI)
                    f = open("result.txt", "a")
                    f.write(str(idList[tempindex]) + "\n")
                    f.close()
